...
public partial class alglib
{
    ...
    *************************************************************************/
    ...
    public static void odesolverrkck(double[] y, int n, double[] x, int m, double eps, double h, out odesolverstate state, xparams _params)
    {
        state = new odesolverstate();
    ...
    public static void odesolverrkck(double[] y, double[] x, double eps, double h, out odesolverstate state, xparams _params)
    {
        int n;
        int m;
    ...
    *************************************************************************/
    ...
    public static bool odesolveriteration(odesolverstate state, xparams _params)
    {
    ...
    *************************************************************************/
    ...
    public static void odesolversolve(odesolverstate state, ndimensional_ode_rp diff, object obj, xparams _params)
    {
        if (diff == null)
            throw new alglibexception("ALGLIB: error in 'odesolversolve()' (diff is null)");
        while (odesolveriteration(state, _params))
        {
            if (state.needdy)
            {
    ...
    *************************************************************************/
    public class odesolverstate : alglibobject
    {
        ...
        {
            innerobj = new odesolver.odesolverstate();
        }
        ...
        {
            get => innerobj.needdy;
            set => innerobj.needdy = value;
        }

        public double[] y => innerobj.y;

        public double[] dy => innerobj.dy;
        ...
        public odesolver.odesolverstate innerobj { get; }

        public override alglibobject make_copy()
        {
            return new odesolverstate((odesolver.odesolverstate)innerobj.make_copy());
        }
    }
    ...
    *************************************************************************/
    public class odesolverreport : alglibobject
    {
        ...
        {
            innerobj = new odesolver.odesolverreport();
        }
        ...
        public odesolver.odesolverreport innerobj { get; }

        public override alglibobject make_copy()
        {
            return new odesolverreport((odesolver.odesolverreport)innerobj.make_copy());
        }
    }
}
    ...
    public class odesolver
    {
        ...
        SEE ALSO
          ...
          -- ALGLIB --
             Copyright 01.09.2009 by Bochkanov Sergey
        *************************************************************************/
        public static void odesolverrkck(double[] y, int n, double[] x, int m, double eps, double h, odesolverstate state, xparams _params)
        {
            ...
            ap.assert(apserv.isfinitevector(y, n, _params), "ODESolverRKCK: Y contains infinite or NaN values!");
            ap.assert(apserv.isfinitevector(x, m, _params), "ODESolverRKCK: Y contains infinite or NaN values!");
            ap.assert(math.isfinite(eps), "ODESolverRKCK: Eps is not finite!");
            ap.assert(eps != 0, "ODESolverRKCK: Eps is zero!");
        ...
        *************************************************************************/
        public static bool odesolveriteration(odesolverstate state, xparams _params)
        {
            ...
            //
            ap.assert(state.h > 0, "ODESolver: internal error");
            ap.assert(m > 1, "ODESolverIteration: internal error");

            //
            // choose solver
            //
            if (state.solvertype != 0) goto lbl_1;
            ...
            state.rka[0] = 0;
            state.rka[1] = 1 / (double)5;
            state.rka[2] = 3 / (double)10;
            state.rka[3] = 3 / (double)5;
            state.rka[4] = 1;
            state.rka[5] = 7 / (double)8;
            state.rkb = new double[6, 5];
            state.rkb[1, 0] = 1 / (double)5;
            state.rkb[2, 0] = 3 / (double)40;
            state.rkb[2, 1] = 9 / (double)40;
            state.rkb[3, 0] = 3 / (double)10;
            state.rkb[3, 1] = -(9 / (double)10);
            state.rkb[3, 2] = 6 / (double)5;
            state.rkb[4, 0] = -(11 / (double)54);
            state.rkb[4, 1] = 5 / (double)2;
            state.rkb[4, 2] = -(70 / (double)27);
            state.rkb[4, 3] = 35 / (double)27;
            state.rkb[5, 0] = 1631 / (double)55296;
            state.rkb[5, 1] = 175 / (double)512;
            state.rkb[5, 2] = 575 / (double)13824;
            state.rkb[5, 3] = 44275 / (double)110592;
            state.rkb[5, 4] = 253 / (double)4096;
            state.rkc = new double[6];
            state.rkc[0] = 37 / (double)378;
            state.rkc[1] = 0;
            state.rkc[2] = 250 / (double)621;
            state.rkc[3] = 125 / (double)594;
            state.rkc[4] = 0;
            state.rkc[5] = 512 / (double)1771;
            state.rkcs = new double[6];
            state.rkcs[0] = 2825 / (double)27648;
            state.rkcs[1] = 0;
            state.rkcs[2] = 18575 / (double)48384;
            state.rkcs[3] = 13525 / (double)55296;
            state.rkcs[4] = 277 / (double)14336;
            state.rkcs[5] = 1 / (double)4;
            ...
            xc = state.xg[0];
            for (i_ = 0; i_ <= n - 1; i_++) state.ytbl[0, i_] = state.yc[i_];

            for (j = 0; j <= n - 1; j++) state.escale[j] = 0;
            ...
            //
            for (j = 0; j <= n - 1; j++) state.escale[j] = Math.Max(state.escale[j], Math.Abs(state.yc[j]));
            ...
            //
            for (i_ = 0; i_ <= n - 1; i_++) state.yn[i_] = state.yc[i_];

            for (i_ = 0; i_ <= n - 1; i_++) state.yns[i_] = state.yc[i_];
            ...
            for (i_ = 0; i_ <= n - 1; i_++) state.y[i_] = state.yc[i_];

            for (j = 0; j <= k - 1; j++)
            {
                v = state.rkb[k, j];
                for (i_ = 0; i_ <= n - 1; i_++) state.y[i_] = state.y[i_] + v * state.rkk[j, i_];
            }
            ...
            v = h * state.xscale;
            for (i_ = 0; i_ <= n - 1; i_++) state.rkk[k, i_] = v * state.dy[i_];

            //
            // update YN/YNS
            //
            v = state.rkc[k];
            for (i_ = 0; i_ <= n - 1; i_++) state.yn[i_] = state.yn[i_] + v * state.rkk[k, i_];

            v = state.rkcs[k];
            for (i_ = 0; i_ <= n - 1; i_++) state.yns[i_] = state.yns[i_] + v * state.rkk[k, i_];
            ...
            if (h2 < h / odesolvermaxshrink) h2 = h / odesolvermaxshrink;
            ...
            //
            xc = xc + h;
            for (i_ = 0; i_ <= n - 1; i_++) state.yc[i_] = state.yn[i_];
            ...
            // save result
            //
            for (i_ = 0; i_ <= n - 1; i_++) state.ytbl[i, i_] = state.yc[i_];
        ...
        OUTPUT PARAMETERS:
          ...
          -- ALGLIB --
             Copyright 01.09.2009 by Bochkanov Sergey
        *************************************************************************/
        public static void odesolverresults(odesolverstate state, ref int m, ref double[] xtbl, ref double[,] ytbl, odesolverreport rep,
            xparams _params)
        {
            double v = 0;
            var i = 0;
            ...
            if (rep.terminationtype > 0)
            {
                ...
                v = state.xscale;
                for (i_ = 0; i_ <= state.m - 1; i_++) xtbl[i_] = v * state.xg[i_];
        ...
        *************************************************************************/
        private static void odesolverinit(int solvertype, double[] y, int n, double[] x, int m, double eps, double h, odesolverstate state,
            xparams _params)
        {
            var i = 0;
            double v = 0;
            ...
            if (m == 1)
            {
                ...
                for (i_ = 0; i_ <= n - 1; i_++) state.ytbl[0, i_] = y[i_];
            ...
            {
                v = Math.Abs(x[1] - x[0]);
                for (i = 2; i <= m - 1; i++) v = Math.Min(v, Math.Abs(x[i] - x[i - 1]));
            ...
            {
                state.xscale = -1;
                for (i_ = 0; i_ <= m - 1; i_++) state.xg[i_] = -1 * state.xg[i_];
            }

            state.yc = new double[n];
            for (i_ = 0; i_ <= n - 1; i_++) state.yc[i_] = y[i_];
        ...
        public class odesolverstate : apobject
        {
            ...
            {
                var _result = new odesolverstate();
                _result.n = n;
                _result.m = m;
        ...
        public class odesolverreport : apobject
        {
            ...
            public override apobject make_copy()
            {
                var _result = new odesolverreport();
                _result.nfev = nfev;
